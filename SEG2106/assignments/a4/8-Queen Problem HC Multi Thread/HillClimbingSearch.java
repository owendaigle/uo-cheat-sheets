...
public class HillClimbingSearch implements Runnable{
    ...
    public void runSearch(){
        ...
        while (presentHeuristic != 0) {
            if (Thread.currentThread().isInterrupted()) { //checks if we are interrupted in which case we do not have to continue computing. 
                break;
            }
            //  Get the next board
    ...
    public void run() {
        runSearch();
        //if it gets here it found the solution
        

        //checks if was interupted
        if (!Thread.currentThread().isInterrupted()) 
        {
            //print solution and check if solution already printed.
            //if solution not already printed, set variable to true
            if (found.compareAndSet(false, true)) printState(getFinalSolution());
        }
