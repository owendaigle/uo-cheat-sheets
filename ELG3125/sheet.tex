...
\begin{document}
	...
	Fourier Transforms is a very important concept in this class. It is used as the basis for the Laplace transform, and Z transform. 
	...
	\begin{array}{|c|c|}
		...
		te^{-at}u(t) \Longleftrightarrow \jmath \frac{\mathrm d }{\mathrm d \omega} \left[\frac{1}{a+\jmath\omega}\right]&\\
	...
	\begin{mdframed}
		\textbf{Ex. } We have the following equation. Get $y(t)$ 
		
		\begin{align*}
			H(\jmath\omega) = \frac{\jmath\omega + 2}{(\jmath\omega+1)(\jmath\omega+3)} \qquad x(t) = e^{-t}u(t)
		\end{align*}
		We start by converting $x(t)$ to $X(\jmath\omega)$, and then we can use the convolution property of $Y(\jmath\omega) = H(\jmath\omega)X(\jmath\omega)$.
		\begin{align*}
			X(\jmath\omega) &= \frac{1}{\jmath\omega+1}\\
			Y(\jmath\omega) &= \left[\frac{\jmath\omega + 2}{(\jmath\omega+1)(\jmath\omega+3)}\right] \cdot \frac{1}{\jmath\omega+1}\\
			&= \frac{\jmath\omega+2}{(\jmath\omega+1)^2(\jmath\omega+3)}\\
			& \text{Use partial fractions}\\
			&=\frac{1/4}{\jmath\omega + 1} + \frac{1/2}{(\jmath\omega+1)^2} + \frac{1/4}{\jmath\omega+3}\\
			&\text{Now I can put all three terms into the time domain using the FT pair}\\
			y(t)&=u(t)\left[\frac{1}{4} e^{-t} + \frac{1}{2} te^{-t} - \frac{1}{4} e^{-3t}\right]
		\end{align*}
	\end{mdframed}
	
	
	\begin{mdframed}
		\textbf{Ex. } Get the frequency response $H(\jmath\omega)$ of the following:
		\begin{align*}
			\frac{\mathrm d ^2 y(t)}{\mathrm d t^2} +4 \frac{\mathrm d y(t)}{\mathrm d t} + 3y(t) = \frac{\mathrm d x(t)}{\mathrm d t} + 2x(t)
		\end{align*}
		I will simply convert to frequency domain, and then use the convolution property.
		\begin{align*}
			&(\jmath\omega)^2Y(\jmath\omega) + 4(\jmath\omega)Y(\jmath\omega) + 3Y(\jmath\omega) = (\jmath\omega)X(\jmath\omega) + 2X(\jmath\omega)\\
			\implies & \frac{Y(\jmath\omega)}{X(\jmath\omega)} = H(\jmath\omega) =  \frac{\jmath\omega+2}{(\jmath\omega)^2+4\jmath\omega+3}
		\end{align*}
	\end{mdframed}
	...
	\begin{mdframed}
		\textbf{Ex. } We have $x[n] = \left(\frac{1}{2}\right)^n u[n]$ and $y[n]=\left(\frac{1}{2}\right)^nu[n] + \frac{1}{2} \left(\frac{1}{4}\right)^nu[n]$. Find the impulse response.
		
		I find the impulse response by knowing that in the frequency domain $H(e^{\jmath\omega})=\frac{Y(e^{\jmath\omega})}{X(e^{\jmath\omega})}$.
		\begin{align*}
			H(e^{\jmath\omega}) &= \left[\frac{1}{1-\frac{1}{2}e^{-\jmath\omega}}\cdot \frac{1}{2}\cdot\frac{1}{1-\frac{1}{4}e^{-\jmath\omega}}\right]\cdot \left(1-\frac{1}{2}e^{-\jmath\omega}\right)\\
			&= ... = \frac{3}{2}\frac{1}{1-\frac{1}{4}e^{-\jmath\omega}}-\frac{1}{2}\frac{e^{-\jmath\omega}}{1-\frac{1}{4}e^{-\jmath\omega}}\\
			& \text{Use FT for the first part, time shift property for second}\\
			\implies h(t)&= \frac{3}{2}\left(\frac{1}{4}\right)^nu[n] - \frac{1}{2}\left(\frac{1}{4}\right)^{n-1}u[n-1]
		\end{align*}
		I can also find the difference equation if I want by going back to $H(e^{\jmath\omega})$ and getting all $Y$ on one side, and $X$ on the other, and then going to time domain. 
		\begin{align*}
			H(e^{\jmath\omega}) = \frac{Y(e^{\jmath\omega})}{X(e^{\jmath\omega})}&= \frac{3}{2}\frac{1}{1-\frac{1}{4}e^{-\jmath\omega}}-\frac{1}{2}\frac{e^{-\jmath\omega}}{1-\frac{1}{4}e^{-\jmath\omega}}=\frac{3-e^{-\jmath\omega}}{2-\frac{1}{2}e^{-\jmath\omega}}\\
			\implies X(e^{\jmath\omega})\left(3-e^{-\jmath\omega}\right) &= Y(e^{\jmath\omega})\left(2-\frac{1}{2}e^{-\jmath\omega}\right)\\
			&\text{Now convert to time domain}\\
			3x[n] - x[n-1] &= 2y[n] - \frac{1}{2}y[n-1]
		\end{align*}
	\end{mdframed}
	...
	\begin{mdframed}
		\textbf{Ex. } Find the bode plot for the following:
		\begin{align*}
			H(\jmath\omega) = 10\times (10\jmath\omega+1)\times\frac{1}{\left(\jmath\omega\right)^2+\jmath\omega+1}\times\left(\frac{\jmath\omega}{10}+1\right)\times \frac{1}{\frac{\jmath\omega}{100}+1}
		\end{align*}
		
		\centering
		\includegraphics[width=0.7\linewidth]{"images/bode ex phase"}
		
		\raggedright
		
		We started by plotting the phase graph for each of the 4 terms (ignore the constant term). We plot a 45 degree slope for the first order, and 90 degree for second order. 
		
		The slope is plotted 1 decade before and after the point. So for the first term at $10^-1$, it goes from $10^{-2}$ to $10^0$. 
		
		Then we sum up all these slopes to get the final result (in Black). 
	...
	\begin{align*}
		...
		& x(t) = \frac{1}{2\pi \jmath} \int^{\sigma + \jmath\infty}_{\sigma - \jmath\infty} X(s) e^{st} \mathrm d t
	\end{align*}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\linewidth]{"images/poles and zeroes"}
		\label{fig:poles-and-zeroes-}
	\end{figure}
	
	Using this graph, we can tell if the function is causal and/or stable. If it is causal, then the ROC will be right sided and unbounded on that side (it will be $\Re\{s\} > a, a\in\mathbb{R}$). If it is stable, then the y axis ($\mathfrak{Im}$) axis will be contained in the ROC. 
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{"images/laplace causal stable"}

		\label{fig:laplace-causal-stable}
	\end{figure}
	
	
	
	We have a table of Laplace transforms as shown below:
	\begin{center}
		\begin{tabular}{|c|c|}
			\hline
			$e^{-at}u(t) \longleftrightarrow \frac{1}{a+s}, \mathfrak{Re}\{s\} > -a$ &  $u(t) \longleftrightarrow \frac{1}{s}, \mathfrak{Re}\{s\} > 0$\\
			\hline
			$-e^{-at} u(-t) \longleftrightarrow \frac{1}{a+s}, \mathfrak{Re}\{s\}<-a$&  $\delta(t) \longleftrightarrow 1$\\
			\hline
			$\cos(\omega_0 t)u(t) \longleftrightarrow \frac{s}{s^2 + \omega_0 ^2}, \mathfrak{Re}\{s\} > 0$& $\sin(\omega_0 t)u(t) \longleftrightarrow \frac{\omega_0}{s^2 + \omega_0 ^2}, \mathfrak{Re}\{s\} > 0$\\
			\hline
		\end{tabular}
	\end{center}
	
	We have a table of laplace properties shown below:
	\begin{center}
		\begin{tabular}{|c|c|}
			\hline
			Time Shifting & $x(t-t_0) \longleftrightarrow e^{-st_0} X(s)$  \\
			\hline
			Flipping & $x(-t) \longleftrightarrow X(-s)$, Inverses ROC\\
			\hline
			S Domain Shifting & $e^{s_0 t}x(t) \longleftrightarrow X(s-s_0)$, ROC is shifted \\
			\hline
			Time Scaling & $x(at) \longleftrightarrow \frac{1}{|a|} X\left(\frac{s}{a}\right)$, ROC is scaled \\
			\hline
			Conjugation& $x^*(t) \longleftrightarrow X^*(s^*)$ \\
			\hline
			Convolution & $x_1(t) * x_2(t) \longleftrightarrow X_1(s) X_2(s)$, ROC is union \\
			\hline
			Differentiation & $\frac{\mathrm d x(t)}{\mathrm d t} \longleftrightarrow sX(s)$\\
			\hline
			S Domain Differentiation & $-tx(t) \longleftrightarrow \frac{\mathrm d X(s)}{\mathrm d s}$\\
			\hline
			Integration & $\int_{-\infty} ^t x(\tau) \mathrm d \tau \longleftrightarrow \frac{1}{s} X(s)$, ROC is $R_{X(s)} \cap \{\mathfrak{Re}\{s\}>0\}$\\
			\hline
		\end{tabular}
	\end{center}
	
	\begin{mdframed}
		\textbf{Ex. }Find $h(t)$ for this differential equation for all cases of the ROC. % do 9.31 from a9.pptx
		\begin{align*}
			\frac{\mathrm d^2 y(t) }{\mathrm d t^2} - \frac{\mathrm d y(t)}{\mathrm d t} - 2 y(t) = x(t)
		\end{align*}
		To do this, I need to get the ROC. This will be found by getting the poles and then finding possible ROCs. 
		
		I start with laplace transform.
		\begin{align*}
			X(s) &= s^2 Y(s) - sY(s) - 2Y(s)\\
			\implies \frac{Y(s)}{X(s)} &= \frac{1}{s^2 - s - 2} = \frac{1}{(s-2)(s+1)}\\
			\implies \frac{Y(s)}{X(s)} &= \frac{1}{(s-2)(s+1)} = \frac{A}{s-2} + \frac{B}{s+1} \qquad \text{Do partial fractions}\\
			\implies 1 =& A(s+1) + B(s-2) \implies 0 = A + B, 1 = A - 2B \implies A = \frac{1}{3}, B = \frac{-1}{3}\\
			\implies \frac{Y(s)}{X(s)} &= \frac{1/3}{s-2} - \frac{1/3}{s+1}
		\end{align*}
		So now I have two poles of $2$ and $-1$ and therefore 3 possible ROCs. 
		
		\centering
		\includegraphics[width=0.4\linewidth]{"images/Laplace ex ROCs"}
	
		\raggedright
		We need to find the equation for each ROC. We use the property that $e^{-at}u(t) \longleftrightarrow \frac{1}{a+s}, \mathfrak{Re}\{s\} > -a$ and $-e^{-at} u(-t) \longleftrightarrow \frac{1}{a+s}, \mathfrak{Re}\{s\}<-a$ are the same, except their ROC is slightly different. 
		\begin{align*}
			&(1) \qquad a < -1\\
			&(2) \qquad-1 < a < 2\\
			&(3) \qquad a > 2
		\end{align*}
		Now I can actually get the equations for these.
		\begin{align*}
			&(1) \qquad h(t) = \frac{1}{3} \left(-e^{2t}u(-t) - (-)e^{-t} u(-t)\right)\\
			&(2) \qquad h(t) = \frac{1}{3} \left(-e^{2t}u(-t) - e^{-t} u(t)\right)\\
			&(3) \qquad h(t) = \frac{1}{3} \left(e^{2t}u(t) - e^{-t} u(t)\right)
		\end{align*}
		We can also say which ROCs are causal, and stable through the properties, or through logic of $h(t)$.
		\begin{align*}
			&(1) \qquad \text{NOT causal, NOT stable}\\
			&(2) \qquad \text{NOT causal, stable}\\
			&(3) \qquad \text{causal, NOT stable}
		\end{align*}
	\end{mdframed}
	
	\section{Z Transform}
	The Z transform is basically the Laplace transform but in discrete time. Rather than using $X(s)$ with $s=\sigma+\jmath\omega$, we have $X(z)$ where $z=re^{\jmath\omega}$ (similar to the discrete fourier transform of $e^{\jmath\omega}$).
	
	By definition, we have it as:
	\begin{align*}
		&X(z) = \sum^\infty_{k=-\infty} x[n] z^{-n} & \text{Z Transform}\\
		&x[n] = \frac{1}{2\pi\jmath}\oint X(z)z^{n-1}\mathrm d z & \text{Inverse Z Transform}
	\end{align*}
	
	We can create the Zero Pole plot as well, and this allows us to see if it is stable and or causal. We however use circles rather than lines (like laplace). If the outside of the circle to the bounds is the ROC, then it is causal. If the unit circle (radius of 1) is in the ROC, then it is stable. 
	
	In the below figure, the first image has the ROC inside the pole (so it does not extend to the bounds), and the unit circle is in the ROC. For the second the ROC extends to the bounds but does not include the unit circle, and the third also extends to the bounds and includes the ROC.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{"images/z stable causal"}
		\label{fig:z-stable-causal}
	\end{figure}
	
	We have a stock list of z transforms, just like the fourier transforms and laplace transforms. 
	\begin{center}
		\begin{tabular}{|c|c|}
			\hline
			$a^nu[n] \longleftrightarrow \frac{1}{1-az^{-1}}, \qquad |z| > |a|$& $-a^nu[-n-1] \longleftrightarrow \frac{1}{1-az^{-1}}, \qquad |z| < |a|$ \\
			\hline
			$u[n] \longleftrightarrow \frac{1}{1-z^{-1}}, \qquad |z| > 1$&  \\
			\hline
		\end{tabular}
	\end{center}
	
	We also have a list of properties of z transforms just like fourier transforms and laplace transforms. 
	\begin{center}
		\begin{tabular}{|c|c|}
			\hline
			Time Shifting & $x[n-n_0] \longleftrightarrow z^{-n_0} X(z)$ \\
			\hline
			Scaling & $z^n_0x[n] \longleftrightarrow X\left(\frac{z}{z_0}\right)$, new ROC is $|z_0|R$\\
			\hline
			Time Reversal & $z[-n] \longleftrightarrow X\left(\frac{1}{z}\right)$, new ROC is $\frac{1}{R}$ \\
			\hline
			Conjugation & $x^*[n] \longleftrightarrow X^*(z^*)$  \\
			\hline
			Convolution & $x_1[n] * x_2[n] \longleftrightarrow X_1(z) X_2(z)$, new ROC is union \\
			\hline
			Differentiation & $nx[n] \longleftrightarrow -z\frac{\mathrm d X(z)}{\mathrm d z}$ \\
			\hline
		\end{tabular}
	\end{center}
	
	\begin{mdframed}
		\textbf{Ex. } Find $H(z)$ for the following:
		
		\begin{align*}
			y[n] - \frac{3}{2} y[n-1] + \frac{1}{2} y[n-1] = x[n]
		\end{align*}
		This is very simple, just use the time shifting property. 
		\begin{align*}
			\implies& Y(z) - \frac{3}{2} z^{-1} Y(z) + \frac{1}{2}z^{-2} Y(z) = X(z)\\
			\implies& H(z) = \frac{Y(z)}{X(z)} = \frac{1}{\frac{1}{2}z^{-2} - \frac{3}{2}z^{-1} + 1}
		\end{align*}
	\end{mdframed}
	...
	\newpage 
	\section{Appendix}
	\includepdf[pages=-]{elg3125_other_formulas_2025}
	\includepdf[pages=-]{elg3125_transform_tables_2025}
\end{document}