...
\begin{document}
	...
	\begin{center}
		\begin{tabular}{|c|c|}
			...
			\hline
			S Domain Shifting & $e^{s_0 t}x(t) \longleftrightarrow X(s-s_0)$ \\
			\hline
			Time Scaling & $x(at) \longleftrightarrow \frac{1}{|a|} X\left(\frac{s}{a}\right)$ \\
			\hline
			Conjugation& $x^*(t) \longleftrightarrow X^*(s^*)$ \\
			\hline
			Convolution & $x_1(t) * x_2(t) \longleftrightarrow X_1(s) X_2(s)$ \\
			\hline
			...
			\hline
			Integration & $\int_{-\infty} ^t x(\tau) \mathrm d \tau \longleftrightarrow \frac{1}{s} X)(s)$\\
			\hline
		\end{tabular}
	\end{center}
